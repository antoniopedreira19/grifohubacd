...
export default function FormularioAplicacao() {
  ...
  const { savePartial, getPartialLeadId } = usePartialLeadCapture("Formulário Aplicação");
  ...
  const handleContinue = () => {
    if (step === 2 && data.setores_atuacao.length === 0) { toast.error("Selecione ao menos um setor."); return; }
    if (step === 3 && !data.ticket_medio) { toast.error("Informe o ticket médio."); return; }
    if (step === 4 && !data.obras_simultaneas) { toast.error("Informe a quantidade de obras."); return; }
    if (step === 5 && !data.estado) { toast.error("Selecione o estado."); return; }
    if (step === 7 && data.setores_equipe.length === 0) { toast.error("Selecione ao menos um setor."); return; }
    if (step === 8 && data.full_name.length < 3) { toast.error("Digite seu nome completo."); return; }
    if (step === 9 && data.phone.replace(/\D/g, "").length < 10) { toast.error("Telefone inválido."); return; }
    if (step === 10 && !data.email.includes("@")) { toast.error("Email inválido."); return; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (partialId) {
        ...
        if (existing) {
          ...
          const ins: Record<string, unknown> = {
            full_name: data.full_name, email: data.email, phone: fullPhone,
            social_media: data.instagram, status: "Novo", origin: "Formulário Aplicação", fbp, fbc,
          };
      ...
      // Form submission (sem product_id)
