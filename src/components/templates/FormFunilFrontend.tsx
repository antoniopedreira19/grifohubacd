...
interface FormFunilFrontendProps {
  productId?: string;
  productSlug?: string;
}
...
export default function FormFunilFrontend({ productId, productSlug }: FormFunilFrontendProps) {
  ...
  const { savePartial, getPartialLeadId } = usePartialLeadCapture("Funil Front-End");
  ...
  /*
   * FRONT-END ORDER:
   * 0: Nome  1: WhatsApp  2: Email  3: Instagram
   * 4: Cargo (cut)  5: Faturamento (cut)
   * 6: Setores atuação  7: Ticket  8: Obras  9: Estado  10: Sócios  11: Setores equipe
   */
  ...
  const handleContinue = () => {
    if (step === 0 && data.full_name.length < 3) { toast.error("Digite seu nome completo."); return; }
    if (step === 1 && data.phone.replace(/\D/g, "").length < 10) { toast.error("Telefone inválido."); return; }
    if (step === 2 && !data.email.includes("@")) { toast.error("Email inválido."); return; }
    if (step === 6 && data.setores_atuacao.length === 0) { toast.error("Selecione ao menos um setor."); return; }
    if (step === 7 && !data.ticket_medio) { toast.error("Informe o ticket médio."); return; }
    if (step === 8 && !data.obras_simultaneas) { toast.error("Informe a quantidade de obras."); return; }
    if (step === 9 && !data.estado) { toast.error("Selecione o estado."); return; }
    if (step === 11 && data.setores_equipe.length === 0) { toast.error("Selecione ao menos um setor."); return; }

    // Partial saves
    if (step === 0) savePartial({ full_name: data.full_name });
    if (step === 1) savePartial({ phone: `${data.countryCode}${data.phone.replace(/\D/g, "")}` });
    if (step === 2) savePartial({ email: data.email });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (partialId) {
        ...
        await supabase.from("leads").update(up).eq("id", partialId);
        leadId = partialId;
      } else {
      ...
      await supabase.from("form_submissions").insert({
        lead_id: leadId, product_id: productId,
        answers: {
          cargo: data.cargo, faturamento: data.faturamento,
          setores_atuacao: data.setores_atuacao, ticket_medio: data.ticket_medio,
          obras_simultaneas: data.obras_simultaneas, estado: data.estado,
          possui_socios: data.possui_socios, setores_equipe: data.setores_equipe,
  ...
  // Step-to-question mapping for display number
  const qNum = step + 1;
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-start md:justify-center bg-[#112232] text-[#E1D8CF] font-sans relative overflow-y-auto p-4 pb-28">
      <style>{`.funil-fe-scroll::-webkit-scrollbar{width:6px}.funil-fe-scroll::-webkit-scrollbar-track{background:transparent}.funil-fe-scroll::-webkit-scrollbar-thumb{background:rgba(225,216,207,0.15);border-radius:3px}.funil-fe-scroll::-webkit-scrollbar-thumb:hover{background:rgba(225,216,207,0.25)}`}</style>
      ...
      <div className="w-full max-w-2xl z-10 flex flex-col items-center pb-20 md:pb-0 pt-6">
        ...
        <div className="w-full relative min-h-[400px]">
          {/* Step 0: Nome */}
          {step === 0 && (
            <QCard icon={<User className="text-[#A47428]" size={32} />} n={qNum} q="Para começarmos, qual é o seu nome completo?" sub="Queremos saber quem está falando conosco.">
          ...
          {/* Step 1: WhatsApp */}
          {step === 1 && (
            <QCard icon={<Phone className="text-[#A47428]" size={32} />} n={qNum} q="Qual seu WhatsApp para contato?" sub="Nossos especialistas entrarão em contato por aqui.">
          ...
          {/* Step 2: Email */}
          {step === 2 && (
            <QCard icon={<Mail className="text-[#A47428]" size={32} />} n={qNum} q="E o seu melhor e-mail?" sub="Para envio de materiais e propostas.">
          ...
          {/* Step 3: Instagram */}
          {step === 3 && (
            <QCard icon={<Instagram className="text-[#A47428]" size={32} />} n={qNum} q="Qual o Instagram da construtora?" sub="Para conhecermos melhor o seu trabalho.">
          ...
          {/* Step 4: Cargo */}
          {step === 4 && (
            <QCard icon={<Briefcase className="text-[#A47428]" size={32} />} n={qNum} q="Qual é o seu cargo atual na empresa?" sub="Selecione a opção que melhor descreve sua posição.">
          ...
          {/* Step 6: Setores de atuação */}
          {step === 6 && (
            <QCard icon={<HardHat className="text-[#A47428]" size={32} />} n={qNum} q="Em quais setores sua empresa atua?" sub="Selecione todos que se aplicam.">
              <div className="grid grid-cols-1 gap-3 mt-4">
          ...
          {/* Step 7: Ticket médio */}
          {step === 7 && (
            <QCard icon={<Hash className="text-[#A47428]" size={32} />} n={qNum} q="Qual o ticket médio dos seus contratos?" sub="Valor aproximado em reais.">
              <div className="grid grid-cols-1 gap-3 mt-4">
          ...
          {/* Step 8: Obras simultâneas */}
          {step === 8 && (
            <QCard icon={<Hash className="text-[#A47428]" size={32} />} n={qNum} q="Quantas obras simultâneas a empresa gerencia?" sub="Número aproximado atual.">
              <div className="grid grid-cols-1 gap-3 mt-4">
                {OBRAS_OPTIONS.map((o) => (
          ...
          {/* Step 9: Região de atuação */}
          {step === 9 && (
            <QCard icon={<MapPin className="text-[#A47428]" size={32} />} n={qNum} q="Qual a região de atuação da empresa?" sub="Selecione a região.">
          ...
          {/* Step 10: Possui sócios */}
          {step === 10 && (
            <QCard icon={<Users className="text-[#A47428]" size={32} />} n={qNum} q="Você possui sócios na empresa?" sub="Sim ou Não.">
          ...
          {/* Step 11: Setores com equipe */}
          {step === 11 && (
            <QCard icon={<ClipboardList className="text-[#A47428]" size={32} />} n={qNum} q="Quais setores da empresa já possuem equipe?" sub="Selecione todos que se aplicam.">
              <div className="grid grid-cols-1 gap-3 mt-4">
